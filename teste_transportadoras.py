...
if __name__ == "__main__":
    # Chama a função e obtém os dois dicionários
    pontuacoes, detalhes_scores = somar_pontuacoes_por_transportadora_origem()

    print("Resultados de pontuação por transportadora e origem do percurso:")
    print("-" * 80)
    
    for (transportadora, origem_percurso), pontuacao in pontuacoes.items():
        print(f"Transportadora: {transportadora} | Origem: {origem_percurso} | Pontuação total: {pontuacao}")
    
    print("\nDetalhes individuais dos scores por transportadora:")
    print("-" * 80)
    
    for (transportadora, origem_percurso), scores in detalhes_scores.items():
        print(f"Transportadora: {transportadora} | Origem: {origem_percurso} | Scores individuais: {scores}")
